...
function resources(){
 layout("resources",`<section class="hero"><div class="eyebrow">Study resources</div><h1>Direct links. Less Googling. 😄</h1><p>Every numbered C# topic has a study link. Use the Learning Path for the complete sequence.</p></section><div class="grid">${ROADMAP.map(p=>`<section class="card phase-card"><div class="eyebrow">${p.phase}</div><h3>${p.icon} ${p.title}</h3>${p.topics.slice(0,5).map(t=>`<p><a class="resource" href="${t.url}" target="_blank">📚 ${String(t.number).padStart(3,'0')} — ${t.title} ↗</a></p>`).join("")}<a class="continue" href="roadmap.html#${p.id}">View phase →</a></section>`).join("")}</div>`);
}
